...
class GridWorld(MDP):
    ...
    """
        ...
        - 1 is an empty cell
        - 0 is a blocked cell
        - 2 is the agent (initial state)
        - 5 is the oil, values is -5
        - 7 is the bump, values is -10
        - 9 is the end, values is +200
    ...
    def create(inarray, rewards):
        # Parse the reward on the first line
        # import ast
        #
        # rewards = ast.literal_eval(rewardlist)

        # Initialize variables
        blocked_cells = []
        initial_state = None
        goals = []
        # Directly assign width from the array's shape
        width = inarray.shape[1]
        row = 0

        # Iterate through the array
        for next_row in inarray:
            column = 0
            for cell in next_row:
                if cell == 0:
                    blocked_cells.append((column, row))
                elif cell == 2:
                    initial_state = (column, row)
                elif cell == 5:
                    goals.append(((column, row), rewards[cell]))
                elif cell == 7:
                    goals.append(((column, row), rewards[cell]))
                elif cell == 9:
                    goals.append(((column, row), rewards[cell]))
        ...
        for y in range(0, self.height):
            for x in range(0, self.width):
                if (x, y) in self.goal_states:
                    if self.goal_states[(x, y)] == -5:
                        img[y][x] = COLOURS['red']
                    elif self.goal_states[(x, y)] == -10:
                        img[y][x] = COLOURS['bisque']
                    elif self.goal_states[(x, y)] == 200:
                        img[y][x] = COLOURS['green']
                elif (x, y) in self.blocked_states:
                    img[y][x] = COLOURS['black']
                elif (x, y) == self.initial_state:
    ...
    def get_next_state(self, current_state, action):
        ...
        # Proposed next state
        proposed_next_state = (current_state[0] + dx, current_state[1] + dy)

        # Check if the proposed next state is within grid boundaries
        if (0 <= proposed_next_state[0] < self.height) and (0 <= proposed_next_state[1] < self.width):
            # Check if the proposed next state is not a blocked state
            if proposed_next_state not in self.blocked_states:
                return proposed_next_state
        # If the proposed next state is outside the grid or blocked, return the current state
        return current_state

    def find_optimal_path(self, policy):
        current_state = self.initial_state
        path = []  # Initialize path without the start state to match the new format

        while True:
            action = policy.select_action(current_state)
            if current_state == self.TERMINAL:
                path.append((current_state, action))  # Include final state and action
                break

            next_state = self.get_next_state(current_state, action)
            if next_state == current_state or any(s == current_state for s, _ in path):
                break

            path.append((current_state, action))  # Append state-action pair
            current_state = next_state

        return path

    def plot_optimal_path(self, path, title="Optimal Path from Start to Goal"):
        fig, ax = plt.subplots()
        # Plot grid and blocked cells
        for y in range(self.height):
            for x in range(self.width):
                if (x, y) in self.blocked_states:
                    ...
                    if self.goal_states[(x, y)] == -5:
                        ...
                        ax.add_patch(plt.Rectangle((x, y), 1, 1, color='bisque'))
                    elif self.goal_states[(x, y)] == 200:
                        ax.add_patch(plt.Rectangle((x, y), 1, 1, color='green'))
                elif (x, y) == self.initial_state:
                    ax.add_patch(plt.Rectangle((x, y), 1, 1, color=(0.53, 0.80, 0.98)))
                else:
                    ax.add_patch(plt.Rectangle((x, y), 1, 1, fill=False))

        for (x, y), action in path:
            # Note: Adjusted the arrow's start point and direction based on (row, column)
            if action == '▼':  # Down
                plt.arrow(x + 0.5, y + 0.5, 0, -0.8, width=0.04, color='black', length_includes_head=True)
            elif action == '▲':  # Up
                plt.arrow(x + 0.5, y + 0.5, 0, 0.8, width=0.04, color='black', length_includes_head=True)
            elif action == '◄':  # Left
                plt.arrow(x + 0.5, y + 0.5, -0.8, 0, width=0.04, color='black', length_includes_head=True)
            elif action == '►':  # Right
                plt.arrow(x + 0.5, y + 0.5, 0.8, 0, width=0.04, color='black', length_includes_head=True)
